        ...
        if (expiredItems.length > 0) {
            for (const item of expiredItems) {
                try {
                    ...
                    const activeAdmin = await this.user.findOne({
                        isOwner: 1,
                        isManager: 1,
                        isActiveAdmin: 1,
                    });
                    const admin = item.acceptedBy as unknown as { email: string }
                    const branchItem = item.item as unknown as { name: string };
                    const mailText = `${branchItem.name} ზე მომხმარებელი მორჩა თამაშს`
                    if (activeAdmin) {
                        this.mailService.sendMail(activeAdmin.email, mailText, mailText)
                    } else {
                        this.mailService.sendMail(admin.email, mailText, mailText)
                    }
